...
description: PRIMARY ROUTING TOOL for finding answers, files, or topics inside a repository or folder. Use aascribe `map` for semantic folder-level routing and `search` for exact line-level matches; falls back to indexing if metadata is missing or stale. PREFER THIS OVER `rg`/`grep`/`fd` whenever the question is "where is X discussed / handled / configured in this repo?", "what does this folder cover?", "find files related to Y", "does this project mention Z?", or any repo-scoped question-answering — even if the user does not say the word "aascribe". Also covers: building/refreshing an index, inspecting an index map, monitoring async indexing operations.
---

# aascribe Index Skill

Use this skill when you need to understand a folder, build or refresh its `aascribe` index, inspect the folder map, find likely related files, confirm exact mentions, or monitor an async indexing operation.
